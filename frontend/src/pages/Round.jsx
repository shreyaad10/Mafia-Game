...
export default function Round() {
  ...
  const { playerId, isHost, socket, setRoom } = useGame();
  ...
  useEffect(() => {
    ...
    socket.on('playerEliminated', ({ room }) => {
      setRoomData(room);
      setRoom(room);
    });
  ...
  return (
    <div className="page page-glow-red" style={{ paddingTop: '1.5rem', paddingBottom: '1.5rem' }}>
      <div className="content-wide animate-fadeIn">
        ...
        {/* My role reminder */}
        {roleInfo && (
          <div className="card animate-scaleIn delay-1" style={{ marginBottom: '1.25rem', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', background: `${roleInfo.color}08`, borderColor: `${roleInfo.color}20` }}>
            <span style={{ fontSize: '1.8rem', filter: `drop-shadow(0 0 8px ${roleInfo.color})` }}>{roleInfo.icon}</span>
            <div>
              <div style={{ fontFamily: 'var(--font-heading)', fontSize: '0.6rem', letterSpacing: '0.3em', color: 'var(--text-dim)', textTransform: 'uppercase', marginBottom: '0.2rem' }}>Your Role</div>
              <div style={{ fontFamily: 'var(--font-heading)', color: roleInfo.color, fontSize: '1rem' }}>{roleInfo.title}</div>
            </div>
          </div>
        )}
          ...
          <div className="card animate-scaleIn delay-2">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              ...
              <span style={{
                ...
                fontSize: '0.8rem',
                color: 'var(--teal)',
                background: 'rgba(66,245,185,0.1)',
            ...
            <div className="player-list">
              {alivePlayers.map((p) => (
                <div key={p._id} className={`player-item ${p.isHost ? 'is-host' : ''}`}>
                  ...
                  <div style={{ display: 'flex', gap: '0.35rem', alignItems: 'center' }}>
                    ...
                    {/* God can eliminate alive players (not themselves) */}
              ...
              {alivePlayers.length === 0 && (
                <p style={{ color: 'var(--text-dim)', fontStyle: 'italic', textAlign: 'center', padding: '1rem' }}>No players alive</p>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div className="card animate-scaleIn delay-3">
              ...
              {deadPlayers.length === 0 ? (
                <p style={{ color: 'var(--text-dim)', fontStyle: 'italic', fontSize: '0.9rem', textAlign: 'center', padding: '0.5rem 0' }}>No casualties yet</p>
              ) : (
                <div className="player-list">
                  {deadPlayers.map((p) => (
            ...
            {isHost && (
              <div className="card animate-scaleIn delay-4" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                >
                  {advancingRound ? 'Advancing…' : `⏭ Next Round (${(roomData?.currentRound || 1) + 1})`}
                </button>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button
                        ...
                        style={{ flex: 1, borderColor: winner === 'Villagers' ? 'var(--teal)' : undefined, color: winner === 'Villagers' ? 'var(--teal)' : undefined }}
                      >
                        🏘️ Villagers
                      </button>
                      <button
                        ...
                        className="btn btn-ghost btn-sm"
                        style={{ flex: 1, borderColor: winner === 'Mafia' ? '#f54242' : undefined, color: winner === 'Mafia' ? '#f54242' : undefined }}
                      >
                        🔪 Mafia
                      </button>
                    </div>
